...
all.symbols = SP500$Ticker.symbol[1:10]
...
# %do% if serial
tic <- Sys.time()
optimal.sd.triggers <- foreach(l=1:(length(all.hist.prices.open)), .combine='c', .packages='quantmod') %do% {
	print(paste(l, " of ", N))
...
####write.csv(df, "results 1-50 and 51-100.csv")
